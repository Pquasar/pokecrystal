...
AI_Smart_EffectHandlers:
	...
	dbw EFFECT_DRAGON_TAIL,      AI_Smart_ForceSwitch
	...
	dbw EFFECT_COIL,             AI_Smart_Curse
	...
	dbw EFFECT_STEALTH_ROCK,     AI_Smart_Spikes
	...
	dbw EFFECT_DRAGON_UP,        AI_Smart_DragonDance
	dbw EFFECT_QUIVER_DANCE,     AI_Smart_DragonDance
...
; discourage if the player's last move was brick break
	ld a, [wLastPlayerCounterMove]
	cp BRICK_BREAK
	jr nz, .checkhp
	inc [hl]
...
AI_Smart_Confuse:
; 90% chance to discourage this move if player's HP is below 50%.
	call AICheckPlayerHalfHP
	ret c
	call Random
	cp 10 percent
...
AI_Smart_Burn:
; 100% chance to discourage this move if player's HP is below 25%.
	call AICheckPlayerQuarterHP
	ret c
	inc [hl]
	inc [hl]
...
; Discourage this move if [enemy's current speed * 2 > player's current speed].
	push hl
...
; Always use this move if the enemy is below 1/4 HP
	call AICheckEnemyQuarterHP
	jp nc, .alwaysuse
...
AI_Smart_FakeOut:
	...
	;Almost always use this move if its enemy's first turn
	ld a, [wEnemyTurnsTaken]
	and a
